...
function link_bin_dir() {
  local dotfiles_dir=$1
  local backupdir="${2}/bin"
  mkdir_not_exist "$backupdir"
...
function link_to_homedir() {
  ...
  if [[ -e "$dotfiles_dir/.linkignore" ]]; then
    mapfile_compat linkignore "$dotfiles_dir/.linkignore"
  fi
  if [[ "$HOME" != "$dotfiles_dir" ]]; then
    ...
    # Link bin directory separately (not prefixed with dot)
    if [[ -d "$dotfiles_dir/bin" ]]; then
      link_bin_dir "$dotfiles_dir" "$backupdir"
    fi
  fi
}
